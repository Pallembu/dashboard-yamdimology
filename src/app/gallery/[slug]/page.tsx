...
export default async function GalleryDetailPage({ params }: Props) {
  try {
    // Fetch gallery data and increment view count
    const [gallery, relatedGalleries] = await Promise.all([
      galleryService.getGalleryBySlug(params.slug),
      galleryService.getGalleryBySlug(params.slug).then(async (gallery) => {
        if (gallery) {
          await galleryService.incrementViewCount(gallery._id)
          return getRelatedGalleries(gallery._id, gallery.category, 4)
        }
        return []
      })
    ])
        ...
        <AnimatedSection className="bg-gradient-to-r from-primary to-primary-dark text-white py-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p className="text-xl text-primary-lighter mb-8">
              Hubungi kami untuk merencanakan perjalanan wisata impian Anda 
              ke {gallery.destination?.name || 'destinasi menakjubkan ini'}
            </p>
